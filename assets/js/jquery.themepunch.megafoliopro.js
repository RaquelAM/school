...
//				}
	...
	function reOrder(container,deep) {
		...
		if (next_layout==11 || next_layout==12 || next_layout==13 || next_layout==15 || next_layout==16)
			if (cw<992  && cw>767) element_amount=3
			  else
			if (cw<767) element_amount=3;
			
		if (next_layout==17)
			if (cw<1200  && cw>767) element_amount=3
			  else
			if (cw<767) element_amount=3;
	...
	function rePosition(container,maxdelay,newentry) {
		...
		container.find('>.mega-entry.tp-layout').each(function(i) {
			...
			if (layout==15 || layout==16)
				if (cw<992 && cw>767) layout=14
				 else
				if (cw<768) layout=14;
				
			if (layout==17)
				if (cw<1200 && cw>767) layout=14
				 else
				if (cw<768) layout=14;
			...
			if (cw>767) {
						...
						if (layout==5) {
								if (cw>767) {
										if (ent.data('child')==0) { w=w400; h=h600; xp=0; 	 }
										if (ent.data('child')==1) { w=w400; h=h300; xp=w400; }
										if (ent.data('child')==2) { w=w400; h=h300; xp=w800; ph=0;  topp=topp+h300;}
										if (ent.data('child')==3) { w=w400; h=h300; xp=w400; 	 }
										if (ent.data('child')==4) {	w=w400; h=h300; xp=w800;  topp=topp+h300;}
